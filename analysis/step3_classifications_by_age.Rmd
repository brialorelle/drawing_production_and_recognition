...
```{r}
# Note that the graphs/csvs in the current paper were generated with a different seed and so may have slightly different jitters/lower and upper bounds of the CIs (within .001)
set.seed(123)
...
ggsave(here::here('data/figures_out/Figure2A_cor_by_category_by_age_jan19.pdf'), width=3, height=3, units='in')

write_csv(cor_by_age, file = here::here('data/figures_csvs/Figure2A_cor_by_category_by_age_jan19.csv'))
```
...
range(lo_all_drawings_by_each_age_count$num_drawings)
sum(lo_all_drawings_by_each_age_count$num_drawings)
```
...
ggsave(filename = here::here('data/figures_out/Figure2B_log_odds_by_category_all_drawings_jan19.pdf'), units = 'in', height= 3, width=5, useDingbats=FALSE)
