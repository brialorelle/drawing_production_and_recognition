...
The final, filtered dataset of N=`r length(unique(all_meta$filename))`  drawings from `r length(unique(all_meta$category))` categories from `r num_subs` children who were on average `r mean(all_meta$age_numeric)` years of age (range 2-10 years). 
...
To assess this, `r length(unique(frequency$subject_id))` parents of children aged 2-10 years filled out a survey asking about the frequency with with their children drew the categories in the dataset.
