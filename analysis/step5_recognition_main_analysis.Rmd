...
write_csv(both_category, file = here::here('data/figures_csvs/Appendix_B3_recognition_by_category.pdf'))
```
...
```{R}
round(out$coefficients,3)
xtable::xtable(summary(out)$coef, digits=3, caption = "Model coefficients of a GLMM predicting visual recognition performance as a function of recognizer age and visual 'distinctiveness' for high-performing children only (i.e. log-odds probability of selecting the correct label in logistic regression based on visual features, see Methods).")
```
